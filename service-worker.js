...
const SHELL_ASSETS = ['/', './', 'index.html', 'app.js', 'manifest.json', 'assets/hero-1280x720.jpg', 'icons/icon-192.png', 'icons/icon-512.png'];
self.addEventListener('install', (e) => { e.waitUntil(caches.open(SHELL_CACHE).then((c) => c.addAll(SHELL_ASSETS))); });
...
self.addEventListener('fetch', (e) => {
  ...
  if (url.hostname === 'api.openweathermap.org') {
    e.respondWith((async () => {
      try { const res = await fetch(e.request); const cache = await caches.open(RUNTIME_WX); cache.put(e.request, res.clone()); return res; }
